...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        # added
        if cfg.common.ctxdict != 'no':
            self.ctx_dict = _pickle.load(open(cfg.common.ctxdict, 'rb'))
            logger.info('load ctxdict from {}'.format(cfg.common.ctxdict))
        else:
            self.ctx_dict = None

        self.ctx_dict_merge = False
        if 'merge' in cfg.common.ctxdict:
            logger.info('using merged ctxdict')
            self.ctx_dict_merge = True
            # rm eos
            self.ctx_dict.pop(task.source_dictionary.eos())

        # if cfg.common.tgtctxdict != 'no':
        #     tgtctx_dict = _pickle.load(open(cfg.common.tgtctxdict, 'rb'))
        #     logger.info('load target ctxdict from {}'.format(cfg.common.tgtctxdict))
        #     # self.tgtctx_dict.pop(task.target_dictionary.eos())
        #     model.decoder.tgtctx_dict = tgtctx_dict
        # else:
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                with maybe_no_sync():
                    # added
                    sample['net_input']['ctx_dict'] = self.ctx_dict
                    # sample['net_input']['tgtctx_dict'] = self.tgtctx_dict
    ...
    # added
    def extract_features_step(self, samples, ctx_dict, layer, clus_type='word',
                    src_or_tgt='src', prob=False, words_to_cluster=None):
        """Do forward pass in evaluation mode."""
        with torch.no_grad():
            self.model.eval()
            ...
            for i, sample in enumerate(samples):  # delayed update loop
                sample, is_dummy_batch = self._prepare_sample(sample)
                if src_or_tgt == 'src':
                    enc_out = self.model.encoder(sample['net_input']['src_tokens'],
                    ...
                    # last layer
                    # B T H
                    state = states[layer-1].transpose(0,1)
                    ...
                    if clus_type == 'word':
                        ...
                        # print('self.task.src_dict.pad()', self.task.src_dict.pad())
                        src_tok = src_tok.cpu().tolist()

                        for il, l in enumerate(src_tok):
                            for iw, w in enumerate(l):
                                if w == self.task.src_dict.eos():
                                    break

                                if words_to_cluster is not None and w not in words_to_cluster:
                                    continue

                                # left pad
                                if w == self.task.src_dict.pad() or self.task.src_dict[w] in set(punctuation):
                                    continue

                                # rt_ctx_dict.append(state[il, iw])
                                if type(ctx_dict) == list:
                                    raise RuntimeError('must dict')
                                    ctx_dict.append(state[il, iw])
                                else:
                                    if w not in ctx_dict:
                                        ctx_dict[w] = [state[il, iw]]
                                    else:
                                        # if len(ctx_dict[w]) == 1000:
                                        #     ctx_dict[w] = [state[il, iw]]
                                        # else:
                                        ctx_dict[w].append(state[il, iw])

                        # return rt_ctx_dict

                    elif clus_type == 'sentence':
                        # B T 1
                        length = sample['net_input']['src_lengths']
                        mask = enc_out['encoder_padding_mask'][0].unsqueeze(-1)
                        state.masked_fill_(mask == 1, 0)
                        state = state.sum(1) / length.float().unsqueeze(-1)
                        # B H
                        state = state.cpu().numpy()
                        ctx_dict.append(state)
                    elif clus_type == 'all':
                        state = state.cpu().numpy()
                        src_tok = sample['net_input']['src_tokens']
                        ...
                        for il, l in enumerate(src_tok):
                            for iw, w in enumerate(l):
                                ...
                                # rm punc
                                if self.task.src_dict[w] not in set(punctuation):
                                    ctx_dict.append(state[il, iw:iw + 1])
                    else:
                        raise RuntimeError('type')

                elif src_or_tgt == 'tgt':
                    # B T H
                    _, extra = self.model.forward(**sample['net_input'],
                                                 return_all_hiddens=True,
                                                 features_only=True)
                    #list[dict] T B H
                    mid_states = extra['inner_states']

                    if clus_type == 'word':

                        # rm eos on the beginning
                        tgt_tok = sample['net_input']['prev_output_tokens']
                        tgt_tok = tgt_tok[:, 1:]
                        # print('self.task.src_dict.pad()', self.task.src_dict.pad())
                        tgt_tok = tgt_tok.cpu().tolist()

                        mid_states = mid_states[layer-1]

                        for name in mid_states:
                            state = mid_states[name]
                            state = state.transpose(0, 1)

                            # state = state[:, 1:]
                            state = state.cpu().numpy()

                            for il, l in enumerate(tgt_tok):
                                # print([self.task.tgt_dict[w] for w in l])

                                for iw, w in enumerate(l):

                                    # right pad
                                    if w == self.task.tgt_dict.eos() or w == self.task.tgt_dict.pad():
                                        break

                                    if self.task.tgt_dict[w] in set(punctuation):
                                        continue

                                    if not prob:
                                        if w not in ctx_dict[name]:
                                            ctx_dict[name][w] = [state[il, iw]]
                                        else:
                                            ctx_dict[name][w].append(state[il, iw])
                                    else:
                                        raise RuntimeError('prob?')
                                        # if w not in ctx_dict:
                                        #     ctx_dict[w] = [(state[il, iw], y_prob[il, iw])]
                                        # else:
                                        #     ctx_dict[w].append((state[il, iw], y_prob[il, iw]))
                                # print(len(ctx_dict[name].keys()))
                    else:
